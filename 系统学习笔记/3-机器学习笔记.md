...
# <span style='color:red'>Day10 下</span>
...
> $$
>
> - 推导要点：$(x_i - \mu)^2 = x_i^2 - 2x_i\mu + \mu^2$
> - 求和后：$\frac{1}{n}\sum{x_i^2} - 2\mu \cdot \frac{1}{n} \sum{x_i} + \mu^2 = \frac{1}{n}\sum{x_i^2} - \mu^2$
>     - $\mu = \frac{1}{n} \sum{x_i}$
...
# <span style='color:red'>Day13 上</span>
...
我们的目标是：
在逻辑回归模型中，根据已有的 (x) 和 (y) 数据，求出最优的决策边界，也就是求解最优参数向量 (W)。
...
$$
z = w_0 + w_1x_1 + w_2x_2 = [1, x_1, x_2][w_0 \ w_1 \ w_2]
$$
...
*   输入数据矩阵 (X) 的维度为 ((10,3))，表示共有 10 组样本，每组有 2 个特征，再加上偏置项；
*   参数向量 (W) 的维度为 ((3,1))；
*   输出向量 (Z) 的维度为 ((10,1))。

### 3.3 Sigmoid 激活函数

通过 Sigmoid 函数将线性输出转为概率预测：
...
这里的运算都是**元素级运算**，对每一个样本单独计算：

*   真实标签向量 (Y) 的维度为 ((10,1))；
*   预测概率向量 (H) 的维度为 ((10,1))；
*   得到的 cost 也是一个 ((10,1)) 的向量。

### 3.5 对损失函数关于参数 (W) 的求导

利用链式法则，对 cost 关于参数 (W) 求导：

$$
\frac{\partial cost}{\partial W} = \frac{\partial C}{\partial H} \cdot \frac{\partial H}{\partial z} \cdot \frac{\partial z}{\partial W}
$$
已知：
...
2. 损失函数对预测值的导数为：
    $$
    \frac{\partial C}{\partial H} = -\frac{Y}{H} + \frac{1 - Y}{1 - H}
    $$
...
$$
\frac{\partial cost}{\partial W} = X^T(H - Y)
$$
...
通过以上推导，可以得到逻辑回归的梯度计算公式：

$$
\frac{\partial cost}{\partial W} = X^T(H - Y)
$$
利用该梯度进行梯度下降更新参数，就可以不断逼近最优参数，从而得到最优的决策边界。
...
<img src="https://gitee.com/rozen_gitee/typora-img/raw/master/img/20260306000000460.png" alt="image-20260306000000253" style="zoom: 50%;" />

这是一个二分类模型的评估结果（比如良性/恶性肿瘤），来自 **`classification_report`**：

- 0 行、1 行：分别表示类别 0 和类别 1 的表现

- precision（精确率）：预测为该类的样本中，有多少是真的

- recall（召回率）：真实属于该类的样本中，有多少被模型找出来

- f1-score（F1 值）：精确率和召回率的综合指标（调和平均），越接近 1 越好

- support：该类别在测试集中真实样本数量

**逐项解读**

- 类别 0

- precision = 0.99：模型把样本判为 0 时，几乎全都是对的（只有约 1% 是错判的）

- recall = 0.96：所有真实为 0 的样本中，96% 被正确识别

- f1-score = 0.97：整体表现非常好

- support = 89：测试集中真实为 0 的样本有 89 个

- 类别 1

- precision = 0.92：预测为 1 的样本中，92% 正确

- recall = 0.98：真实为 1 的样本几乎都被找出来（只漏掉约 2%）

- f1-score = 0.95：同样表现不错

- support = 48：真实为 1 的样本有 48 个

**整体指标**

- accuracy = 0.96：整体正确率 96%（137 个样本中，大约 131 个被预测正确）

- macro avg（宏平均）

- 对每个类别的指标简单平均（不考虑各类样本数量差异）

- precision 0.95, recall 0.97, f1-score 0.96，说明两个类别都学得比较均衡

- weighted avg（加权平均）

- 按照各类别的 support 进行加权平均，更反映整体样本的真实分布

- precision/recall/f1-score 都约为 0.96

**总结**

- 你的模型整体表现非常好（准确率 96%），对两类样本都有比较高的精确率和召回率。

- 如果这是乳腺癌良恶性分类，一般更关心“癌症类”（通常是 1）的 recall，这里达到 0.98，说明几乎不会漏诊，这是非常理想的结果。